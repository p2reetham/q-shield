...
export default function SignDocument({ keys, onSigned }: Props) {
  ...
  const sign = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.sign(keyId, content);
      setResult(res);
      onSigned();
    } catch (e: any) {
      setError(e.message);
    ...
    <div className="panel p-4 flex flex-col">
      ...
      {result && (
        <div className="mt-auto space-y-1 font-mono text-[11px] text-graphite-500 border-t border-graphite-800 pt-3">
          <div className="truncate"><span className="text-graphite-600">Document Hash</span> {result.document_hash}</div>
          <div className="truncate"><span className="text-graphite-600">Signature</span> {result.signature_hex.slice(0, 32)}...</div>
          <div><span className="text-graphite-600">Key ID</span> {result.key_id}</div>
          <div><span className="text-graphite-600">Timestamp</span> {new Date(result.timestamp).toLocaleString()}</div>
          <div><span className="text-graphite-600">Status</span> <span className="text-safe">{result.verification_status}</span></div>
        </div>
      )}
    </div>
  );
